...
# Regex for ffprobe to get dimensions from JSON output
# (Not used directly as we'll parse JSON, but useful for debugging if needed)
# FFPROBE_DIMENSIONS_REGEX = re.compile(r"Stream #0:0\(.*?Video:.*?(\d+)x(\d+)")
...
def _get_image_dimensions(image_path):
    """
    Uses ffprobe to get the width and height of an image.
    Returns (width, height) tuple, or (None, None) if dimensions cannot be retrieved.
    """
    cmd = [
        "ffprobe",
        "-v", "error",
    ...
    try:
        ...
        sys.stderr.write(f"{ERROR_PREFIX} Could not get dimensions for {os.path.basename(image_path)} using ffprobe: {e}\n")
        return None, None
    except Exception as e:
        sys.stderr.write(f"{ERROR_PREFIX} Unexpected error getting dimensions for {os.path.basename(image_path)}: {e}\n")
        return None, None

def _rotate_image_ffmpeg(image_path):
    """
    Rotates a single image 90 degrees anti-clockwise using FFmpeg, overwriting the original.
    """
    temp_path = None
    try:
        # Create a temporary file for the output
        suffix = os.path.splitext(image_path)[1]
        fd, temp_path = tempfile.mkstemp(suffix=suffix, dir=os.path.dirname(image_path))
        os.close(fd)

        cmd = [
            "ffmpeg",
            "-y", # Overwrite output file if it exists (for temp file)
            "-i", image_path,
            "-vf", "transpose=2", # transpose=2 rotates 90 degrees anti-clockwise
            temp_path
        ]
        
        result = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
        os.replace(temp_path, image_path) # Overwrite original with rotated image
        return True
    except FileNotFoundError:
        sys.stderr.write(f"{ERROR_PREFIX} FFmpeg executable not found during rotation of {os.path.basename(image_path)}. Skipping rotation.\n")
        if temp_path and os.path.exists(temp_path): os.remove(temp_path)
        return False
    except subprocess.CalledProcessError as e:
        sys.stderr.write(f"{ERROR_PREFIX} FFmpeg rotation failed for {os.path.basename(image_path)}:\n{e.stderr.decode('utf-8', errors='ignore').strip()}\n")
        if temp_path and os.path.exists(temp_path): os.remove(temp_path)
        return False
    except Exception as e:
        sys.stderr.write(f"{ERROR_PREFIX} Unexpected error during rotation of {os.path.basename(image_path)}: {e}\n")
        if temp_path and os.path.exists(temp_path): os.remove(temp_path)
        return False

def _process_image_for_orientation(image_name, folder):
    """
    Worker function to check and rotate a single image if it's portrait.
    Returns the image_name if successful (rotated or not), None otherwise.
    Prints error if processing fails.
    """
    image_path = os.path.join(folder, image_name)
    try:
        width, height = _get_image_dimensions(image_path)
        if width is None or height is None:
            # Error already logged by _get_image_dimensions
            return None # Exclude this image

        # If Width / Height < 1, it's portrait
        if width / height < 1:
            # sys.stdout.write(f"{INFO_PREFIX} Rotating portrait image: {os.path.basename(image_path)} (original: {width}x{height})\n")
            if _rotate_image_ffmpeg(image_path):
                return image_name # Successfully rotated
            else:
                return None # Rotation failed
        else:
            return image_name # No rotation needed

    except Exception as e:
        sys.stderr.write(f"{ERROR_PREFIX} Unexpected error during orientation adjustment for {os.path.basename(image_path)}: {e}\n")
        return None
...
def process_single_archive(archive_file_path, audio_file_path, fps, duration_per_frame, image_extensions, audio_fade_in_duration, audio_fade_out_duration, output_video_dir):
    """
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # --- Image Orientation Adjustment Protocol ---
            print(HEADER_LINE)
            print(f"{ARROW_RIGHT} Initiating Image Orientation Adjustment Protocol...")
            
            # Use thread_map for parallel processing of images for orientation check and rotation
            orientation_results_relative = thread_map(
                lambda img_name: _process_image_for_orientation(img_name, temp_dir),
                extracted_image_paths_relative,
                max_workers=NUM_WORKERS,
                desc=f"{ARROW_RIGHT} Orienting Data Streams",
                unit="image",
                ncols=100
            )
            
            # Filter out None results (failed adjustments)
            images_after_orientation_adjustment = [img_name for img_name in orientation_results_relative if img_name is not None]

            print(f"\n{ARROW_LEFT} Orientation Adjustment Protocol complete. {len(images_after_orientation_adjustment)} images oriented for verification.")
            if len(images_after_orientation_adjustment) < len(extracted_image_paths_relative):
                 print(f"{WARNING_PREFIX} {len(extracted_image_paths_relative) - len(images_after_orientation_adjustment)} images were excluded due to orientation adjustment errors.")
            print(HEADER_LINE)

            if not images_after_orientation_adjustment:
                 print(f"{ERROR_PREFIX} No images available after attempting orientation adjustment.")
                 return False
            ...
            verify_results_relative = thread_map(
                 ...
                 images_after_orientation_adjustment, # Use images after orientation adjustment
                 max_workers=NUM_WORKERS,
            ...
            if len(final_images_for_ffmpeg_relative) < len(images_after_orientation_adjustment):
                print(f"{WARNING_PREFIX} {len(images_after_orientation_adjustment) - len(final_images_for_ffmpeg_relative)} data streams failed integrity check and were excluded.")
            print(HEADER_LINE)


            if not final_images_for_ffmpeg_relative:
                print(f"{ERROR_PREFIX} No usable images found after all checks (orientation adjustment and FFmpeg verification).")
                return False
...
def main():
    ...
    try:
        ...
        print(f"{INFO_PREFIX} FFprobe module detected. {STATUS_OK}")
    except (FileNotFoundError, subprocess.CalledProcessError):
         print(f"{INFO_PREFIX} FFprobe module not found. {STATUS_FAIL}")
         print(f"{ERROR_PREFIX} Please ensure FFprobe (usually bundled with FFmpeg) is installed and accessible in your system's PATH.")
         sys.exit(1)
